...
export default function Checkout() {
  ...
  const startOnlinePayment = async (orderData) => {
    try {
      ...
      // Try sending receipt but don't block order placement if it fails
      try {
        await api.post("/api/send-receipt/", orderData);
      } catch (receiptErr) {
        console.warn("Receipt sending failed:", receiptErr);
      }
      ...
      dispatchCartUpdate();
      setShowPopup(true);
      setPendingOrder(null); // Clear after success
    } catch (err) {
      console.error(err);
      alert("Online payment failed");
      setShowQR(false); // Close QR on error
      setProcessing(false);
    }
  };
    ...
    <>
      <div className="container py-5" style={{ maxWidth: "900px" }}>
        ...
        <div className="text-center">
          <button className="btn btn-success px-5" onClick={placeOrder} disabled={processing}>
        ...
        <div className="modal fade show"
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 text-center">
              ...
              <p className="text-muted">
                {paymentMethod === "cod" ? "Amount to be Paid (COD)" : "Amount Paid"}: ₹{totalAmount.toFixed(2)} (incl. GST)
              </p>
