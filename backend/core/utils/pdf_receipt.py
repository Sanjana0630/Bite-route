...
def generate_receipt_pdf(order):
    ...
    is_cod = order['payment_method'].lower() == "cash on delivery"
    payment_status_text = f"Payment Method: {order['payment_method']}"
    total_text = f"Total Amount {'to be Paid (COD)' if is_cod else 'Paid'}: ₹ {order['total']}"

    c.drawString(40, y, payment_status_text)

    y -= 15
    c.drawString(40, y, total_text)
