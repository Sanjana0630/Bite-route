...
def send_receipt_to_email(request):
    try:
        ...
        if "@" not in registered_email or "." not in registered_email:
            print(f"⚠️ Skipping email: '{registered_email}' is not a valid email address.")
            return Response({
                "status": False,
                "message": f"'{registered_email}' is not a valid email address. Order saved, but receipt not sent."
            })
        ...
        import traceback
        print("❌ ERROR SENDING RECEIPT:")
        traceback.print_exc()
        return Response(
            {"message": f"Failed to send receipt: {str(e)}"},
            status=500
        )
